...
namespace RAM_4bit
{
    ...
    inline void boot_up(std::vector<int>& program)
    {
        ...
        add_ruleset(rules);
        Turing::Turing_Machine<states, symbols, 200> machine(go, end, rules); //Create a Machine with 200 tape segments that will end when state "end" is reached and that uses the rule set defined above
